...
def sparse_submanifold_conv_fwd_implicit_gemm_splitk_kernel(
    ...
    for k in range(k_start, k_end):
        ...
        neighbor_offset_n = tl.load(neighbor + offset_n.to(tl.int64) * V + v).to(tl.int64)                   # (B1,)
    ...
    out_ptr = output + block_id_k.to(tl.int64) * N * Co + (out_offset_n[:, None] * Co + out_offset_co[None, :])
